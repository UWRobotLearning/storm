...
number_of_friction_cone_violations = {
    str(get_ensemble_size(agent)): {} for agent in vf_agents}
...
max_ee_linear_vel_norm = {str(get_ensemble_size(agent)): {}
                          for agent in vf_agents}
max_ee_angular_vel_norm = {str(get_ensemble_size(agent)): {}
                           for agent in vf_agents}
    ...
    for model_filename in vf_agents:
        ...
        for pred_temp in prediction_temps:
            ...
            try:
                ...
                print(
                    f"Metrics for {model_filename} with pred_temp {pred_temp}: {metrics}")
                metrics_dict[str(ensemble_size)][pred_temp] = metrics
                filtered_episodes_1 = [
                    ep for ep in metrics if ep['abs_cube_pos_change'] < 0.02]
                filtered_episodes_2 = [
                    ep for ep in metrics if ep['abs_cube_pos_change'] < 0.1]
                num_successes_1 = sum(episode['success']
                                      for episode in filtered_episodes_1)
                num_successes_2 = sum(episode['success']
                                      for episode in filtered_episodes_2)
                success_1[str(ensemble_size)][pred_temp] = (
                    num_successes_1/len(metrics))*100
                success_2[str(ensemble_size)][pred_temp] = (
                    num_successes_2/len(metrics))*100

                successful_episodes_1 = [
                    episode for episode in filtered_episodes_1 if episode['success'] == 1]
                if successful_episodes_1:
                    mean_number_of_steps_successful = sum(
                        episode['number_of_steps'] for episode in successful_episodes_1) / len(successful_episodes_1)
                else:
                    # or handle the case when there are no successful episodes
                    mean_number_of_steps_successful = 0
                number_of_steps[str(ensemble_size)][int(
                    pred_temp)] = mean_number_of_steps_successful
                ee_dist_error[str(ensemble_size)][int(pred_temp)] = sum(episode['dist_err_final']
                                                                        for episode in filtered_episodes_1) / len(filtered_episodes_1) if len(filtered_episodes_1) > 0 else 0
                rotation_error[str(ensemble_size)][int(pred_temp)] = sum(episode['rot_err_final']
                                                                         for episode in filtered_episodes_1) / len(filtered_episodes_1) if len(filtered_episodes_1) > 0 else 0

                successful_episodes_2 = [
                    episode for episode in filtered_episodes_2 if episode['success'] == 1]
                if successful_episodes_2:
                    mean_number_of_steps_successful_2 = sum(
                        episode['number_of_steps'] for episode in successful_episodes_2) / len(successful_episodes_2)
                else:
                    mean_number_of_steps_successful_2 = 0

                max_ee_linear_vel_norm[str(ensemble_size)][pred_temp] = max(
                    episode['ee lin vel twist max'] for episode in successful_episodes_1) if successful_episodes_1 else 0
                max_ee_angular_vel_norm[str(ensemble_size)][pred_temp] = max(
                    episode['ee ang vel twist max'] for episode in successful_episodes_1) if successful_episodes_1 else 0
                max_angular_tilt[str(ensemble_size)][pred_temp] = max(
                    episode['tilt_angle_max'] for episode in successful_episodes_1) if successful_episodes_1 else 0

                print("success (hard)", (num_successes_1/len(metrics))*100)
                print("success (soft)", (num_successes_2/len(metrics))*100)
                print("mean_number_of_steps_successful",
                      mean_number_of_steps_successful)
                print("ee lin vel twist max",
                      max_ee_linear_vel_norm[str(ensemble_size)][pred_temp])
                print("ee ang vel twist max",
                      max_ee_angular_vel_norm[str(ensemble_size)][pred_temp])
                print("angular_tilt", max_angular_tilt[str(
                    ensemble_size)][pred_temp])
...
if plot:
    ...
    ax = sns.heatmap(df_success, annot=True, cmap="YlGnBu",
    ...
    plt.figure(figsize=(10, 6))
    ax = sns.heatmap(df_success_2, annot=True, cmap="YlGnBu",
                     cbar=True, annot_kws={"size": 10})
    for text in ax.texts:
        text.set_text(custom_formatter(float(text.get_text())))

...
class RobotExperienceCollector():
    def __init__(self, cfg, policy, device=torch.device('cpu')):
        self.cfg = cfg
        ...
        self.reset_policy = MPCPolicy(obs_dim=obs_dim, act_dim=act_dim, config=self.reset_mpc_cfg.mpc, device=self.device)

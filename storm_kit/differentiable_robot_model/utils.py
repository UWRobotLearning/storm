...
@torch.jit.script
def cross_product(vec3a:torch.Tensor, vec3b:torch.Tensor) -> torch.Tensor:
...
def bfill_lowertriangle(A: torch.Tensor, vec: torch.Tensor)->torch.Tensor:
    ii, jj = np.tril_indices(A.size(-2), k=-1, m=A.size(-1))
    A[..., ii, jj] = vec
    return A


def bfill_diagonal(A: torch.Tensor, vec: torch.Tensor)->torch.Tensor:

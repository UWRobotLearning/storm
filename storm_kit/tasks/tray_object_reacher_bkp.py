...
class TrayObjectReacherBkp(ArmTask):
    """
    ...
    def __init__(self, cfg, world_params=None, viz_rollouts=False, device=torch.device('cpu')):
        super(TrayObjectReacherBkp, self).__init__(cfg=cfg,
        ...
        self.init_buffers()
        # for friction cost
        self.m = 0.5 #mass of cube
        self.c = torch.tensor([0.0, 0.0, 0.025])  #com from origin of tray
        s = 0.05  # Side length of the cube
        half_s = s / 2
        self.c_vectors = torch.tensor([
            [half_s, half_s, -half_s],
            [-half_s, half_s, -half_s],
            [half_s, -half_s, -half_s],
            [-half_s, -half_s, -half_s]
        ])
        self.J = torch.tensor([[0.05, 0.0, 0.0], [0.0, 0.05, 0.0], [0.0, 0.0, 0.05]])
        self.mu = 0
        # jitFrictionCost = torch.jit.script(FrictionConeCost)
        # self.friciton_cost = jitFrictionCost(self.m, self.c, self.J, self.mu, self.c_vectors, device=self.device)
        # jitFrictionCost = FrictionConeCost(self.m, self.c, self.J, self.mu, self.c_vectors, device=self.device)
        # self.friciton_cost = torch.jit.script(jitFrictionCost)

        self.friction_cost = FrictionConeCost(self.m, self.c, self.J, self.mu, self.c_vectors, device=self.device)
    ...
    def compute_cost(
        ...
        cost, cost_terms = super(TrayObjectReacherBkp, self).compute_cost(
        ...
        print("friction cone cost time", time.time() - s)
    ...
    def update_params(self, param_dict):
        ...
        super(TrayObjectReacherBkp, self).update_params(retract_state=retract_state)

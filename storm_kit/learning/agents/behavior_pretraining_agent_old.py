...
class BPAgent(Agent):
    def __init__(
        ...
        if self.qf is not None:
            self.critic_optimizer = optim.Adam(self.qf.parameters(), 
                                        lr=self.cfg['critic_optimizer']['lr'])

        if self.vf is not None:
            self.critic_optimizer = optim.Adam(self.vf.parameters(), 
                                        lr=self.cfg['critic_optimizer']['lr'])
    ...
    def update(self, batch_dict, step_num):
        ...
        if self.vf is not None:
            vf_loss, vf_info_dict = self.compute_vf_loss(batch_dict)
            # total_loss += qf_loss
        ...
        if self.qf is not None:
            qf_loss, qf_info_dict = self.compute_qf_loss(batch_dict)
            # total_loss += qf_loss
    ...
    def compute_policy_loss(self, batch_dict):
        ...
        if isinstance(policy_out, torch.distributions.Distribution):  # MLE
            bc_loss = -1.0 * policy_out.log_prob(act_batch).mean()
            ...
            bc_loss = F.mse_loss(new_actions, act_batch, reduction=None).mean()
        ...
        return bc_loss, policy_info_dict
    
    def compute_qf_loss(self, batch_dict):
    ...
    def compute_vf_loss(self, batch_dict:Dict[str, torch.Tensor]):
        vf_info_dict = {}
        return torch.tensor(0.0), vf_info_dict

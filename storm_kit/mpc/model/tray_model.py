...
class SixDOFTrayRobotModel(nn.Module):
    def __init__(
            self,
            batch_size: int = 1000,
            horizon: int = 5,
        ...
        self.n_dofs = 3  # 6 degrees of freedom for a 6-DOF tray robot
        ...
        if self.num_traj_points <= 1:
            ...
            smooth_blending = torch.linspace(dt_traj_params['base_dt'], dt_traj_params['max_dt'],
                                            steps=int((1 - dt_traj_params['base_ratio']) * self.num_traj_points)).tolist()
    ...
    def tensor_step(self,
        ...
        state_seq = self.step_fn(state, act, state_seq, self._dt_h, self._integrate_matrix, self._fd_matrix,
                                 self.n_dofs, self.num_instances, batch_size, horizon)
    ...
    def rollout_open_loop(self, start_state_dict: Dict[str, torch.Tensor], act_seq: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        ee_state_seq = ee_state_seq.view((self.num_instances, curr_batch_size, num_traj_points, 10))
        ee_pos = ee_state_seq[:, :, :, 0:3]
        ee_vel = ee_state_seq[:, :, :, 3:6]
        ee_acc = ee_state_seq[:, :, :, 6:9]

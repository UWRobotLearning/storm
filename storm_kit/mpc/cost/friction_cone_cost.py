...
class FrictionConeCost(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.m = kwargs['m']
        self.d = torch.tensor(kwargs['com'], device=self.device)
        self.J = torch.tensor(kwargs['J'], device=self.device)
        self.mu = kwargs['mu']
        self.g = kwargs.get('gravitational_constant', 9.81)
        self.friction_cost_weight = kwargs['friction_cost_weight']
        self.tangential_cost_weight = kwargs['tangential_cost_weight']
        self.contact_points = torch.tensor(kwargs['contact_points'], device=self.device)
        self.debug = False
        self.init_tensors()
    ...
    def forward(self, v_dot: torch.Tensor, omega: torch.Tensor, omega_dot: torch.Tensor, R: torch.Tensor) -> torch.Tensor:
        ...
        tangential_force_norm =  l2_norm_ft #logcosh(l2_norm_ft)
        ...
        total_cost = self.friction_cost_weight*friction_cost.sum(dim=-1) + self.tangential_cost_weight*tangential_force_norm.sum(dim=-1) #sum over all contact points
        return 1 * total_cost
